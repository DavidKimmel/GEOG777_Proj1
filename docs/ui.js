...
(function () {
  ...
  function setBusy(isBusy) {
    ...
    const sensBtn = document.getElementById('loadSens');
    const spin = document.getElementById('spinner');
    if (loadBtn)  { loadBtn.disabled = isBusy;  loadBtn.textContent  = isBusy ? 'Loading...'  : 'Load Data'; }
    if (sensBtn) { sensBtn.disabled = isBusy; sensBtn.textContent = isBusy ? 'Loading...' : 'Load Sensitivity'; }
    if (spin) {
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadTracts(k = null) {
      const gjFile = k ? `outputs/tracts_k${k}.geojson` : 'outputs/tracts_base.geojson';
      const gj = await fetch(gjFile).then(r => r.json());
      ...
      tractsLayer = L.geoJSON(gj, {
        ...
        onEachFeature: (feature, layer) => {
          ...
          const nitr = Number(feature.properties?.mean_nitrate);
          layer.bindPopup(`<b>Tract</b> ${feature.properties?.GEOID10 ?? ''}<br/>Cancer rate: ${fmt(v)}<br/>Mean nitrate: ${fmt(nitr)} mg/L`);
        }
      });
    ...
    async function loadSensitivity() {
      setBusy(true);
      try {
        setStatus('Loading sensitivity data...');

        // Load pre-computed sensitivity CSV
        const csvUrl = 'outputs/sensitivity_neighbors12_cell1000.csv';
        ...
        for (let i = 1; i < lines.length; i++) {  // Skip header
          ...
          if (parts.length >= 8) {
            rows.push({
              k: parseFloat(parts[0]),
              r2: parseFloat(parts[1]),
              slope: parseFloat(parts[2]),
              intercept: parseFloat(parts[3]),
              p_value: parseFloat(parts[4]),
              ci_low: parseFloat(parts[5]),
              ci_high: parseFloat(parts[6]),
              n: parseInt(parts[7])
            });
          }
        }
        ...
        setStatus('Sensitivity loaded.');
      } catch (err) {
        console.error(err);
        setStatus('Error loading sensitivity data.');
      }
    ...
    if (sensBtn) sensBtn.addEventListener('click', loadSensitivity);
    ...
    async function loadPrecomputedData() {
      setBusy(true);
      try {
        setStatus('Loading pre-computed data...');
        const k = parseFloat(kSelect.value);

        // Build file paths
        const tag = k.toFixed(1);
        const pngUrl = `outputs/nitrate_k${tag}.png`;
        const csvUrl = `outputs/tract_table_k${tag}.csv`;
        const gjUrl = `outputs/tracts_k${tag}.geojson`;
        const boundsUrl = `outputs/bounds.json`;

        // Load bounds
        const bounds = await fetch(boundsUrl).then(r => r.json());

        // Load CSV and compute OLS stats
        const csvText = await fetch(csvUrl).then(r => r.text());
        const lines = csvText.trim().split('\n');
        const data = [];
        for (let i = 1; i < lines.length; i++) {  // Skip header
          const parts = lines[i].split(',');
          if (parts.length >= 3) {
            const nitr = parseFloat(parts[1]);
            const canr = parseFloat(parts[2]);
            if (!isNaN(nitr) && !isNaN(canr)) {
              data.push({ nitrate: nitr, canrate: canr });
            }
          }
        }
        ...
        const n = data.length;
        if (n < 2) throw new Error('Not enough data for OLS');

        const meanX = data.reduce((s, d) => s + d.nitrate, 0) / n;
        const meanY = data.reduce((s, d) => s + d.canrate, 0) / n;

        let num = 0, denX = 0, denY = 0;
        for (const d of data) {
          num += (d.nitrate - meanX) * (d.canrate - meanY);
          denX += (d.nitrate - meanX) ** 2;
          denY += (d.canrate - meanY) ** 2;
        }

        const slope = num / denX;
        const intercept = meanY - slope * meanX;
        const r2 = (num ** 2) / (denX * denY);

        // Compute p-value and confidence interval (simplified)
        const residuals = data.map(d => d.canrate - (intercept + slope * d.nitrate));
        const sse = residuals.reduce((s, r) => s + r ** 2, 0);
        const mse = sse / (n - 2);
        const slopeStdErr = Math.sqrt(mse / denX);
        const tStat = slope / slopeStdErr;

        // Approximate p-value (two-tailed)
        const df = n - 2;
        const p_value = 2 * (1 - jStat_tcdf(Math.abs(tStat), df));  // Simplified

        // 95% CI
        const tCrit = 1.96;  // Approximate for large samples
        const ci_low = slope - tCrit * slopeStdErr;
        const ci_high = slope + tCrit * slopeStdErr;

        // Determine units
        const maxRate = Math.max(...data.map(d => d.canrate));
        const is_proportion = maxRate <= 1.0;
        const rate_units = is_proportion ? 'proportion' : 'raw';
        const rate_scale = is_proportion ? 100000 : 1;
        ...
        // Reload tracts with k-specific data
        await loadTracts(tag);

        handleRunResult(result, k);
    ...
    function jStat_tcdf(t, df) {
      // Very rough approximation for p-value calculation
      // For better results, you'd want a proper stats library
      const x = df / (df + t * t);
